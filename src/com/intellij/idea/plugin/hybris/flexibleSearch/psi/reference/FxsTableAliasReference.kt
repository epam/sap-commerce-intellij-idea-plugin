...
import com.intellij.idea.plugin.hybris.flexibleSearch.psi.reference.result.FxsTableAliasResolveResult
...
class FxsTableAliasReference(owner: FlexibleSearchSelectedTableName) : PsiReferenceBase.Poly<FlexibleSearchSelectedTableName>(owner) {
    ...
    companion object {
        val CACHE_KEY =
            Key.create<ParameterizedCachedValue<Array<ResolveResult>, FxsTableAliasReference>>("HYBRIS_FXS_CACHED_REFERENCE")

        private val provider = ParameterizedCachedValueProvider<Array<ResolveResult>, FxsTableAliasReference> { ref ->
            ...
            val result: Array<ResolveResult> = PsiTreeUtil.getParentOfType(ref.element, FlexibleSearchSelectCoreSelect::class.java)
                ...
                ?.let { arrayOf(FxsTableAliasResolveResult(it)) }
