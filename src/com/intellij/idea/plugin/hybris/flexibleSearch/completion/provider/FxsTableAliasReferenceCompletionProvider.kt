...
class FxsTableAliasReferenceCompletionProvider : CompletionProvider<CompletionParameters>() {

    override fun addCompletions(parameters: CompletionParameters, context: ProcessingContext, result: CompletionResultSet) {
        val project = parameters.editor.project ?: return
        val psiElementUnderCaret = parameters.originalPosition ?: parameters.position

        if (!isTableNameIdentifier(parameters)) return
    ...
    companion object {
        val instance: CompletionProvider<CompletionParameters> = ApplicationManager.getApplication().getService(FxsTableAliasReferenceCompletionProvider::class.java)
    }
