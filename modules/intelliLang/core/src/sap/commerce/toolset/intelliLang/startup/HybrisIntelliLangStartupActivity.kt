...
package sap.commerce.toolset.intelliLang.startup
...
class HybrisIntelliLangStartupActivity : ProjectActivity {
    ...
    override suspend fun execute(project: Project) {
        ...
        CoroutineScope(Dispatchers.Default).launch {
            ...
            val clazz = smartReadAction(project) { JavaPsiFacade.getInstance(project).findClass(HybrisConstants.CLASS_FQN_FLEXIBLE_SEARCH_QUERY, allScope) }
                ?: return@launch
            val fsqExtends = smartReadAction(project) {
                ClassInheritorsSearch
                    .search(clazz, allScope, true)
                    .findAll()
                    .mapNotNull { it.name }
                    .toSet()
            }
