...
/**
 * Provides bounded completion for logger names by combining matching
 * packages and classes on demand.
 *
 * The lookup is wired to match the full text up to the caret as a
 * single prefix — [getPrefix] returns the entire text rather than the
 * last identifier — so typing `com.example.User` filters against the
 * full FQN. Class candidates are also registered with their short
 * name as an additional lookup string when the user has not typed a
 * dot, so typing `User` matches `com.example.UserService` via short
 * name while typing `com.example.User` matches by FQN.
 *
 * Each refresh resolves only the current prefix and limits visible
 * results to [MAX_VISIBLE_SUGGESTIONS]; an extra non-selectable
 * "… N more matches" hint is added whenever the cap is exceeded so
 * the user knows to refine their search. PSI access is wrapped in a
 * read action, and class lookup is split into a short-name
 * collection phase followed by per-name resolution to avoid nested
 * stub-index calls.
 *
 * Dots are accepted unconditionally via [acceptChar] so typing
...
internal class CxLoggerNameCompletionProvider(
    ...
    override fun fillCompletionVariants(
        ...
        runReadAction {
            ...
            collectMatchingClasses(prefix, scope, state)
        }
    ...
    private fun collectMatchingClasses(
        ...
        for (shortName in matchingShortNames) {
            ProgressManager.checkCanceled()
            val keepGoing = cache.processClassesWithName(shortName, { psiClass ->
                ProgressManager.checkCanceled()
