...
class CngNamespaceNotOptimized : XmlSuppressableInspectionTool() {
    ...
    override fun buildVisitor(holder: ProblemsHolder, isOnTheFly: Boolean): PsiElementVisitor = object : XmlElementVisitor() {
        override fun visitXmlAttribute(attribute: XmlAttribute) {
            ...
            val xmlAttribute = attribute.takeIf { it.name.startsWith("xmlns:") } ?: return
            val parentTag = attribute.parent
            // do not process root namespaces
            if (parentTag.parent is XmlDocument) return
