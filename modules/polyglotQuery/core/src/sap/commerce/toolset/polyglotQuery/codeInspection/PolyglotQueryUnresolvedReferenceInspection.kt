...
class PolyglotQueryUnresolvedReferenceInspection : LocalInspectionTool() {
    ...
    private class PolyglotQueryPsiVisitor(private val problemsHolder: ProblemsHolder) : PolyglotQueryVisitor() {
        ...
        override fun visitTypeKeyName(element: PolyglotQueryTypeKeyName) = problemsHolder.verifyReferences(element) {
            i18n("hybris.inspections.pgq.unresolved.type.key", canonicalText)
        }

        override fun visitAttributeKeyName(element: PolyglotQueryAttributeKeyName) = problemsHolder.verifyReferences(element) {
            i18n("hybris.inspections.pgq.unresolved.attribute.key", canonicalText)
        }
    }
}
