...
class FxSImpExParamTest {
    ...
    fun render_plainAttribute() {
        val param = FxSImpExParam(attributeName = "code")
        assertEquals("code", param.render())
    }

    @Test
    fun render_withNestedPath() {
        val param = FxSImpExParam(attributeName = "catalogVersion", nestedPath = "catalog(id),version")
        assertEquals("catalogVersion(catalog(id),version)", param.render())
    }

    @Test
    fun render_withSingleModifier() {
        val param = FxSImpExParam(attributeName = "code", modifiers = listOf("unique=true"))
        assertEquals("code[unique=true]", param.render())
    }

    @Test
    fun render_withMultipleModifiers() {
        val param = FxSImpExParam(attributeName = "name", modifiers = listOf("lang=en", "unique=true"))
    ...
    fun render_emptyNestedPath_skipsParens() {
        val param = FxSImpExParam(attributeName = "code", nestedPath = "")
        assertEquals("code", param.render())
    }
    ...
    fun formatValue_emptyString_passThrough() {
        val param = FxSImpExParam(attributeName = "code", attributeType = "java.lang.String")
    ...
    fun formatValue_localizableString_wrapsInDoubleQuotes() {
        val param = FxSImpExParam(attributeName = "name", attributeType = "localizableString")
        assertEquals("\"My Product\"", param.formatValue("My Product"))
    }

    @Test
    fun formatValue_stringType_escapesEmbeddedDoubleQuotes() {
        val param = FxSImpExParam(attributeName = "description", attributeType = "java.lang.String")
        // Input:  say "hello"
        // Output: "say ""hello"""
    ...
    fun formatValue_integerType_passThrough() {
        val param = FxSImpExParam(attributeName = "priority", attributeType = "java.lang.Integer")
        assertEquals("42", param.formatValue("42"))
    }

    @Test
    fun formatValue_unknownType_passThrough() {
        val param = FxSImpExParam(attributeName = "pk")
