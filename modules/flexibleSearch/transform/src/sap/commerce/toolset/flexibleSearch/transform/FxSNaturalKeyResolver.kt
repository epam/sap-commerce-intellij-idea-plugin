...
object FxSNaturalKeyResolver {
    ...
    private fun resolveInternal(
        ...
        if (depth >= FxSTypeSnapshot.MAX_DEPTH) return TSConstants.Attribute.PK

        val uniqueKeys = snapshot.uniqueIndexKeys
        if (uniqueKeys.isEmpty()) {
            return if (snapshot.hasCodeAttr) ATTR_CODE else TSConstants.Attribute.PK
        }
    ...
    private fun findUniqueIndexKeys(meta: TSGlobalMetaItem): List<String> {
        val fromIndexes = meta.allIndexes
            .filter { it.isUnique }
            .map { it.keys.filterNot { key -> key.equals(TSConstants.Attribute.PK, ignoreCase = true) }.toList() }
