...
class CxPropertiesTreeModel(
    private val rootTreeNode: CxPropertiesTreeNode,
) : BaseTreeModel<CxPropertiesTreeNode>(), Disposable, InvokerSupplier {

    private val nodes = mutableMapOf<CxPropertiesNode, CxPropertiesTreeNode>()
    ...
    override fun getChildren(parent: Any?) = parent
        ...
        ?.userObject
        ?.asSafely<CxPropertiesNode>()
        ?.getChildren()
        ?.onEach { it.update() }
        ?.map { nodes.computeIfAbsent(it) { CxPropertiesTreeNode(it) } }

    override fun getInvoker() = invoker

    fun reload(path: TreePath) = treeStructureChanged(path, null, null)

    override fun dispose() = nodes.clear()
}
