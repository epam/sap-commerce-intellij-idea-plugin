...
class CxRemotePropertyStateView(private val project: Project) : Disposable {
            ...
            override fun compute(): DialogPanel {
                ...
                row {
                    ...
                    button("Apply Property") {
                        ...
                        CxRemotePropertyStateService.getInstance(project).upsertProperty(addKeyField.text.trim(), addValueField.text) { ok ->
                            if (ok) {
                                addKeyField.text = ""
                                addValueField.text = ""
                            }
    ...
    suspend fun render(coroutineScope: CoroutineScope, properties: Collection<CxPropertyPresentation>?): JComponent {
        val viewPanel = lazyViewPanel.value

        if (properties == null) {
            toggleView(showFetchProperties)
            return viewPanel
        }
    ...
    private fun renderData() {
        ...
        val view = if (filtered.isEmpty()) {
            noDataView("No properties match the current filter.")
        } else panel {
            row {
                label("Key").bold()
            ...
            filtered.forEach { property ->
                ...
                row {
                    ...
                    button("Apply") {
                        CxRemotePropertyStateService.getInstance(project).upsertProperty(property.key, valueField.text)
                    }

                    button("Delete") {
                        CxRemotePropertyStateService.getInstance(project).deleteProperty(property.key)
                    }
                }.layout(RowLayout.PARENT_GRID)
            }
        }
    ...
    private fun toggleView(vararg unhide: AtomicBooleanProperty) = listOf(showFetchProperties, showDataPanel)
