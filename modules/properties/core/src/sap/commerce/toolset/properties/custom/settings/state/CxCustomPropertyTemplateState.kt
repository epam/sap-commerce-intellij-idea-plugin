...
data class CxCustomPropertyTemplateState(
    ...
    fun mutable() = Mutable(
        ...
        properties = AtomicProperty(properties.map { it.mutable() }),
    )

    data class Mutable(
        val uuid: String,
        val name: ObservableMutableProperty<String>,
        val properties: ObservableMutableProperty<List<CxCustomPropertyState.Mutable>>,
    ) {
        fun immutable() = CxCustomPropertyTemplateState(
            uuid = uuid,
            name = name.get(),
            properties = properties.get().map { it.immutable() },
        )
    }
}
