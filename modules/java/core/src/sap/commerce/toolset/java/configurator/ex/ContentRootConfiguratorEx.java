...
public final class ContentRootConfiguratorEx {
    ...
    private static void configureCommonRoots(
        ...
        if (customModuleDescriptor
            || !rootProjectDescriptor.getImportSettings().isImportOotbModulesInReadOnlyMode()
            ...
            if (customModuleDescriptor || !rootProjectDescriptor.getImportSettings().isExcludeTestSources()) {
    ...
    private static void excludeCommonNeedlessDirs(
        ...
        if (isCustomModuleDescriptor(moduleDescriptor)
            || !moduleDescriptor.getRootProjectDescriptor().getImportSettings().isImportOotbModulesInReadOnlyMode()) {
    ...
    private static void configureWebRoots(
        ...
        if (isCustomModuleDescriptor(moduleDescriptor) || !rootProjectDescriptor.getImportSettings().isImportOotbModulesInReadOnlyMode()) {
    ...
    private static void configureWebInf(
        ...
        if (isCustomModuleDescriptor(moduleDescriptor)
            || (!moduleDescriptor.getRootProjectDescriptor().getImportSettings().isImportOotbModulesInReadOnlyMode() && testSrcDirectoriesExists(rootDirectory))
        ) {
