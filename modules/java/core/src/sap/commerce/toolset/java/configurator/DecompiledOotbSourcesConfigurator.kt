...
class DecompiledOotbSourcesConfigurator : ProjectPostImportConfigurator {
    ...
    override fun postImport(hybrisProjectDescriptor: HybrisProjectDescriptor) {
        ...
        if (appSettings.decompiledOotbSourcesConsentAsked) return
        if (!hybrisProjectDescriptor.isWithDecompiledOotbSources) return
        ...
        ApplicationManager.getApplication().invokeAndWait({
            ...
            hybrisProjectDescriptor.isWithDecompiledOotbSources = accepted
    ...
    override suspend fun asyncPostImport(hybrisProjectDescriptor: HybrisProjectDescriptor) {
        ...
        val tasks = collectTargets(hybrisProjectDescriptor)
            .ifEmpty { return }
        ...
        if (succeeded > 0 || failed > 0) {
            ...
            Notifications.info(
                ...
                "Completed: $succeeded, Failed: $failed. Output: ${HybrisConstants.DOC_DECOMPILED_SOURCES_PATH}"
    ...
    private fun collectTargets(hybrisProjectDescriptor: HybrisProjectDescriptor): List<JarTask> {
        if (hybrisProjectDescriptor.project == null) return emptyList()

        val modules = hybrisProjectDescriptor.moduleDescriptorsToImport.values
            .filterIsInstance<YModuleDescriptor>()
            .filter { it.descriptorType === ModuleDescriptorType.OOTB
                || it.descriptorType === ModuleDescriptorType.PLATFORM
                || it.descriptorType === ModuleDescriptorType.EXT }
        val tasks = mutableListOf<JarTask>()
        modules.forEach { descriptor ->
            if (descriptor is YSubModuleDescriptor) return@forEach
            val moduleRoot = descriptor.moduleRootDirectory
            ...
            val targetRoot = File(moduleRoot, HybrisConstants.DOC_DECOMPILED_SOURCES_PATH)
